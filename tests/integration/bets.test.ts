...
describe('POST /bets', () => {
  ...
  describe('should respond with a status 404', () => {
    it('if the participant ID is an invalid number greater than zero.', async () => {
      const { id: gameId } = await buildStartGame();
      const bet = generateBet({ amountBet: 2000, gameId, participantId: 1 });
      const { status, body } = await server.post('/bets').send(bet);
      expect(status).toBe(httpStatus.NOT_FOUND);
      expect(body).toEqual({});
    });
    it('if the game ID is an invalid number greater than zero.', async () => {
      const { id: participantId } = await buildParticipant(1500);
      const bet = generateBet({ amountBet: 2000, gameId: 1, participantId });
      const { status, body } = await server.post('/bets').send(bet);
      expect(status).toBe(httpStatus.NOT_FOUND);
      expect(body).toEqual({});
    });
  });
  describe(`should respond with a status 422`, () => {
    ...
    it('if fields are invalid.', async () => {
      const { id: participantId } = await buildParticipant(1500);
      const { id: gameId } = await buildStartGame();
  ...
  it(`should respond with a status 400 if the participant's balance is below the bet.`, async () => {
    const { id: gameId } = await buildStartGame();
    const { id: participantId } = await buildParticipant(1500);
